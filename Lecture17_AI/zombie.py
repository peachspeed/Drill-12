...
class Zombie:
    ...
    def get_patrol_location(self):
        """
        순찰 경로에서 현재 위치 번호에 해당하는 목표 지점을 설정하고,
        다음 목표 지점을 가리키도록 loc_no를 업데이트합니다.
        """
        self.tx, self.ty = self.patrol_locations[self.loc_no]
        self.loc_no = (self.loc_no + 1) % len(self.patrol_locations)  # 다음 경로로 이동
        return BehaviorTree.SUCCESS
    ...
    def build_behavior_tree(self):
        ...
        # 행동 트리 설정
        self.bt = BehaviorTree(chase_or_patrol)
